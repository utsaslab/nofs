/*
 * mke2bpfs.c - Make a ext2bpfs filesystem.
 *
...
/* mke2fs.c changed to add backpointers to the ext2 filesystem.
 Vijay (vijayc@cs.wisc.edu)
*/
...
#include "ext2fs/ext2bp_fs.h"
#include "et/com_err.h"
#include "uuid/uuid.h"
#include "e2p/e2p.h"
#include "ext2fs/ext2bpfs.h"
#include "util.h"
#include "profile.h"
#include "prof_err.h"
#include "../version.h"
...
const char * program_name = "mke2bpfs";
...
static void create_lost_and_found(ext2_filsys fs)
{
	...
	/* Vijay: This results to lost+found having 4 blocks in its inode. However,
  	 * the blocks do not contain any meaningful data - They contain random 
         * information */

	/* Switching this off for the time being since it doesn't add any 
	 * meaningful functionality, but causes the ext2bp dir check to fail */
...
static int mke2fs_setup_tdb(const char *name, io_manager *io_ptr)
{
	...
	if (!tdb_file)
		goto alloc_fn_fail;
	sprintf(tdb_file, "%s/mke2bpfs-%s.e2undo", tdb_dir, device_name);
